...
function OurServices() {
  ...
  return (
    ...
    >
      <div
        className={`my-8 flex w-11/12 flex-col items-center justify-center gap-12 md:w-9/12 ${
          isVisible ? "animate-scale-up" : "opacity-0"
        }`}
      >
        {/* Cabeçalho da seção */}
        <header className="flex flex-col items-center justify-center gap-4 text-center text-dark transition-opacity duration-700">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {services.map((service, index) => (
            ...
            >
              <span className="material-icons text-4xl text-dark">
                {service.icon}
              </span>{" "}
              {/* Ícone do serviço */}
              <h3 className="text-center text-xl font-semibold uppercase md:text-2xl">
                {service.name}
              </h3>{" "}
              {/* Nome */}
              <p className="text-center text-base opacity-75 md:text-lg">
                {service.description}
              </p>{" "}
              {/* Descrição */}
            </a>
